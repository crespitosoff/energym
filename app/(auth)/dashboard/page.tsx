  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [statsRes, expiringRes, registerRes] = await Promise.all(fetches)
        const [statsJson, expiringJson, registerJson] = await Promise.all([
          statsRes.json(), expiringRes.json(), registerRes.json(),
        ])
        if (statsJson.data) setStats(statsJson.data)
        if (expiringJson.data) setExpiring(expiringJson.data)
        if (registerJson.data) setRegister(registerJson.data)
      } catch {
          ...
          <div className="p-3.5 rounded-xl bg-emerald-500/5 border border-emerald-500/20 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <p className="text-sm text-emerald-400 font-medium">
                Caja abierta — Base: {formatMoney(register.initial_cash)}
              </p>
            </div>
            <Link href="/caja" className="text-xs text-emerald-400/70 hover:text-emerald-300 font-medium transition-colors">
              Gestionar →
            </Link>
          </div>
        ) : (
          <Link href="/caja" className="block p-3.5 rounded-xl bg-amber-500/5 border border-amber-500/20 hover:bg-amber-500/10 transition-colors">
            <div className="flex items-center gap-2.5">
              ...
              <p className="text-sm text-amber-400 font-medium">Caja cerrada — Toca para abrir</p>
            </div>
          </Link>
        )}
        ...
        {role && ['admin', 'asistente'].includes(role) && (
          <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <Link href="/caja" className="card-interactive flex flex-col items-center gap-3 text-center !py-5">
                <div className="w-11 h-11 rounded-xl bg-emerald-500/10 flex items-center justify-center text-emerald-400">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.375c0-.621.504-1.125 1.125-1.125H20.25M2.25 6v9m18-10.5v.75c0 .414.336.75.75.75h.75m-1.5-1.5h.375c.621 0 1.125.504 1.125 1.125v9.75c0 .621-.504 1.125-1.125 1.125h-.375m1.5-1.5H21a.75.75 0 00-.75.75v.75m0 0H3.75m0 0h-.375a1.125 1.125 0 01-1.125-1.125V15m1.5 1.5v-.75A.75.75 0 003 15h-.75M15 10.5a3 3 0 11-6 0 3 3 0 016 0zm3 0h.008v.008H18V10.5zm-12 0h.008v.008H6V10.5z" />
                  </svg>
                </div>
                <p className="text-xs font-medium text-white/70">Caja</p>
              </Link>
